...
CDBPool::CDBPool(const char* pool_name, const char* db_server_ip, uint16_t db_server_port, 
	...
	:m_nDbServerPort(db_server_port), m_nDbMaxConnCnt(max_conn_cnt), m_sPoolName(pool_name), m_sDbServerIp(db_server_ip),  
	m_sUserName(username?username:""), m_sPassword(password?password:""), m_sDbName(db_name)
{
	...
	m_pDBPool = new acl::mysql_pool(addr, m_sDbName.c_str(), m_sUserName.c_str(), m_sPassword.c_str(), m_nDbMaxConnCnt);
	if (m_pDBPool)
	{
...
int CDBPool::Init()
{
	...
	log("db pool: %s, size: %d", m_sPoolName.c_str(), m_nDbMaxConnCnt);
	return 0;
}
...
int CDBManager::Init(const string& configFileName)
{
	...
	char* db_instances = config_file.GetConfigName("MsgSvrMysqlDbInstances");
