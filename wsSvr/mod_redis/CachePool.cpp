...
bool CacheConn::isExistsKeys(list<string> keys,list<string>& existKeys)
{
	...
	list<string>::iterator pKey = keys.begin();
	
	for(; pKey != keys.end(); pKey++)
	{
		redisAppendCommand(m_pContext, "EXISTS %s", pKey->c_str());
...
bool CacheConn::delKeys(list<string> keys)
{
	bool ret = true;
	...
	for(; pKey != keys.end(); pKey++)
	{
		redisAppendCommand(m_pContext, "DEL %s", pKey->c_str());
		i++;
	}
...
bool CacheConn::hgetAllBykeys(list<string> keys,map<string,map<string,string>>& ret_value)
{
	if (Init()) {
		return false;
	}
	...
	for(; pKey != keys.end(); pKey++)
	{
		redisAppendCommand(m_pContext, "HGETALL %s", pKey->c_str());
	}
	...
	for(; pKey != keys.end(); pKey++)
	{
		...
		if (!reply) {
			log("key no exist:%s", pKey->c_str());
			ret_value.insert(make_pair(*pKey,fields));
			
		}
		else if ( (reply->type == REDIS_REPLY_ARRAY) && (reply->elements % 2 == 0) ) {
			for (size_t i = 0; i < reply->elements; i += 2) {
				redisReply* field_reply = reply->element[i];
				redisReply* value_reply = reply->element[i + 1];
...
long CacheConn::lpop(string key,string& value)
{
	if (Init()) {
		return -1;
	}
	...
	{
		ErrLog("lpop key[%s] no member",key.c_str());
		freeReplyObject(reply);
		return -1;
	}
...
long CacheConn::lrem(string key,string value)
{
...
long CacheConn::rmpush(string key, list<string>& values)
{
	if (Init()) {
		return -1;
	}

	list<string>::iterator pValue = values.begin();
	for(; pValue != values.end(); pValue++)
	{
		redisAppendCommand(m_pContext, "RPUSH %s %s", key.c_str(), (*pValue).c_str());
	}
