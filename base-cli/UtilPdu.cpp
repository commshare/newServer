...
void CByteStream::WriteInt8(uchar_t *buf, int8_t data)
{
	buf[0] = static_cast<uchar_t>(data);
}

void CByteStream::WriteUint8(uchar_t *buf, uint8_t data)
{
	buf[0] = static_cast<uchar_t>(data);
}
...
void CByteStream::WriteFixedLengthString(uchar_t* buf, const char* str, uint32_t len)
{
	if(!str)
		return;
...
void CByteStream::ReadFixedLengthString(string& str, uint32_t len)
{
	char buf[64];
