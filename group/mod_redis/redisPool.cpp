...
int CRedisManager::Init(CConfigFileReader* pReader)
{
	...
	char* db_instances = config_file.GetConfigName("GrpSvrRedisInstances");
	...
	for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++)
	{
		...
		snprintf(retry_inter, 64, "%s_retryinterval", pool_name);
		snprintf(redirect_max, 64, "%s_redirect_max", pool_name);
		snprintf(redirect_sleep, 64, "%s_redirect_sleep", pool_name);
		snprintf(addrs, 64, "%s_cluster", pool_name);
		snprintf(maxconncnt, 64, "%s_maxconncnt", pool_name);
		snprintf(conn_timeout, 64, "%s_conntimeout", pool_name);
		snprintf(rw_timeout, 64, "%s_rwtimeout", pool_name);
		...
		DbgLog("redis init, addr = %s", db_addrs);
