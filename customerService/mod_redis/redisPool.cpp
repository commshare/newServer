...
:m_pCluster(cluster)
{
	m_listCmd.set_cluster(m_pCluster, 100);
}
...
string CRedisConn::lpop(const char* key)
{
	m_listCmd.clear();
	acl::string buf;
	m_listCmd.lpop(key, buf);
	return buf.c_str();
}

int CRedisConn::rpush(const char* key, const std::vector<acl::string>& values)
{
	m_listCmd.clear();
	return m_listCmd.rpush(key, values);
}
