...
#define UNIONAB(_A, _B) _A#_B
#define XPRINTF(fmt, ...) printf(UNIONAB("%s:%s[%d]", fmt), __FILE__, __FUNCTION__, __LINE__, ##__VA_ARGS__);\
							printf("\r\n");\
							fflush(stdout);
							

/*#define XPRINTF(fmt, ...) string path = __FILE__;\
							string filename;\
							int pos = path.find_last_of('/');\
							filename = path.substr(pos+1);\
							printf(UNIONAB("%s[%d]:%s ", fmt), filename.c_str(), __LINE__, __FUNCTION__, ##__VA_ARGS__);\
							printf("\r\n");\
							fflush(stdout);
							*/
#endif
