...
bool CLoginCache::UpdateUserRec(acl::string sUserId, UserCache_t &rec)
{
	std::map<acl::string, acl::string> mFields;
	char tBuf[30];
	memset(tBuf,0,30);
	...
	m_hashRedis.clear();
    acl::string key = sUserId;
	if (m_hashRedis.hmset(key.c_str(), mFields) == false)
	{
...
bool CLoginCache::InsertUserRec(acl::string sUserId, UserCache_t &rec)							 // Insert a new user record into redis cache 
{
...
std::shared_ptr<CLoginInfo> CLoginCache::GetLoginInfo(const std::string& userId)
{
	if(userId.empty())
	{
	...
	//std::string pcUserId = "PC_" + userId;
	std::string cmUserId = userId;
    ...
    std::vector<acl::string> result;
	if (m_hashRedis.hmget(cmUserId.c_str(), fields, &result))
	{
		if (!result[1].empty() && !result[2].empty())
		{

			DbgLog("user %s Login info ,status = %s, cmip = %s, cmport = %s device_token=%s",
                   userId.c_str(), result[0].c_str(), result[1].c_str(), result[2].c_str(), result[3].c_str());
			return std::shared_ptr<CLoginInfo>(new CLoginInfo( result[0].c_str(), result[1].c_str(), result[2].c_str(), result[3].c_str()));
		}
	}
