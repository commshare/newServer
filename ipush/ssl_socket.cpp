...
bool CSslSocket::Init(CSslEventDispatch *pSslEventDispatch , const char *sslCeFileName , const char *sslRsaPkFileName)
{
	...
	{
		ErrLog("pSslEventDispatch is nullptr");
		return false;
	}
	...
	if (!m_sslCeFileName.empty())
	{
		if(!(nRet = SSL_CTX_use_certificate_file(m_sslCtx, m_sslCeFileName.c_str(), SSL_FILETYPE_PEM)))
		{
			...
			ErrLog("init ssl: use certificate file failed.errno: %d.", nErrorCode);
			SSL_CTX_free(m_sslCtx);
			m_sslCtx = NULL;
			return false;
		}	 
...
int CSslSocket::_SslShakeHands()
{
	...
	if (1 != nRet)
	{
		...
		ErrLog("SSL_connect failed, socket:%d ssl err:%d, errno %d: %s", m_socket, iRet, errno, strerror(errno));
		return -1;
	}
	...
	{
		WarnLog("SSL_get_certificate error!");
		return -1;
	}
...
int CSslSocket::Send(void* buf, int len)
{
	...
	//return -1;
	//CAutoLock autoLock(&m_mutexSendLock);
	...
	if (n <= 0)
	{
		...
		{
			ErrLog("!!!send failed, error code: %d", errno);
			n =  -1;
		}
...
int CSslSocket::Close()
{
	...
	InfoLog("socket:%d, close", m_socket);
...
int CSslSocket::SslConnectWebSite(callback_t callback, void* callback_data, const char* sHost, const int16_t iPort)
{
	...
	{
		ErrLog("get host %s Ip addr failed", sHost);
	...
	InfoLog("connect host %s, ip:%s, port:%u", m_strHost.c_str(), m_remote_ip.c_str(), m_remote_port);
	...
	InfoLog("tcp connect success, local port:%d, addr:%s",	m_local_port, m_local_addr.c_str());
