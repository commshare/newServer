...
#include"pb/im.pushSvrAPNsMsg.pb.h"
#include"json/json.h"
...
public:
	...
	bool	SetData(const im::PSvrMsg& svrMsg);
...
private:
	...
	//string			m_sTitle_loc_key;			//国际化
	//vector<string>	m_vTitle_loc_args;			//国际化

	//string			m_sAction_loc_key;			//国际化 待确认 不设置

	//string			m_sLoc_key;					//国际化 待确认 不设置
	//vector<string>	m_vLoc_args;				//国际化 待确认 不设置
	...
	//string				sSound;					//播放声音	default don`t set
	//uint32				nContent_available;		//勿扰模式 default don`t set

	/*
	   string				sCategory;				//待确认 不设置
	   string				sThread_id;				//待确认 不设置
	*/
...
public:
	...
	bool Init(/*const char* sHost, */const char* sTopic, const char* sAuthorization);

	bool SetDeviceToken(const char* sDeviceToken);

	//设置jwt的值，调用第三方库libjwthttps://jwt.io
	static bool SetAuthorization(const char *key);
...
//public:
//	CRecvProtoBuf();
//	CRecvProtoBuf(char *buf, int bufLen);
//	~CRecvProtoBuf();
//
//	//解析数据包，设置apns格式数据包
//
//	/**
//	 将上游的protobuf的单个协议包，解析出来，并设置到APNs的包头和包payload当中
//
//	 @param buf 单个protobuf协议包首地址
//	 @param bufLen 协议包大小
//	 @return true or false
//	 */
//	bool SetSendApnsMsg(char *buf, int bufLen);
//
//	void SetMsgId(const string &strMsgid);
...
//	//解析数据包，设置apns格式数据包
//	//内存申请很频繁，需使用内存池
//	//need you to release the memory whit g_bufPool.ReleaseNode();
//	shared_ptr<CApnsPostData> GetSendApnsBufVec();
//
//	bool Init();
//
//private:
//	//内存申请很频繁，需使用内存池
//	CHead					m_head;				//CHead vec for apns
//	CPayLoad				m_payLoad;			//CPayLoad vec for apns
//
//
//	PSvrMsg					m_PSvrMsg;				//protobuf接收的大类
