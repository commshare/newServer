	...
	if (type == ZOO_SESSION_EVENT) 
	{
		if (state == ZOO_CONNECTED_STATE) 		// Success  to connect zookeeper
		{
            ...
            if(fd != -1)
            {
                ...
                CServerLinkMgr* pServerLinkMgr = (CServerLinkMgr*)context;//context== this, init in zookeeper_init();
                pServerLinkMgr->SetLocalHost(strIp);//把探测到的本机IP保存起来，覆盖配置文件里边配置的那个IP
                InfoLog("fd = %d, localSockeAdd:IP = %s, localSockAdd:port = %d", fd, strIp, port);
            }
#endif
			CZookeeper::GetInstance()->SetConnect(true);
#if 0
		    //here do some testing work
//            const char* strHostPort = zoo_get_current_server(zh);
  //          InfoLog("now zk client conneted %s", strHostPort);
            
            struct sockaddr srcSockaddr={0};
            socklen_t len = sizeof(sockaddr);
            
            struct sockaddr* retSockAdd = zookeeper_get_connected_host(zh, &srcSockaddr, &len);
            struct sockaddr_in *sock = (struct sockaddr_in*)retSockAdd;
            ...
            char* strIp = inet_ntoa(sock->sin_addr);
            InfoLog("retSockeAdd:IP = %s, retSockAdd:port = %d", strIp, port);


            sock = (struct sockaddr_in*)&srcSockaddr;
//            port = ntohs(sock->sin_port);
//            in = sock->sin_addr;
//            memset(str, 0, INET_ADDRSTRLEN);
//            inet_ntop(AF_INET, &in, str, sizeof(str));
            port = sock->sin_port;
            strIp = inet_ntoa(sock->sin_addr);
            InfoLog("srcSockaddr:IP = %s, srcSockaddr:port = %d", strIp, port);
#endif 
        } 
...
void CZookeeper::ConnectZookeeper()
{
	...
	while (!m_zHandle && (nRetryTimes < m_nRetryTimes)) 
	{
		...
		m_zHandle = zookeeper_init(m_sZookeeperUrl.c_str(),CZookeeper::Event,m_nTimeout,0, this,0); //Connect to zookeeper
		++nRetryTimes;					
	}
