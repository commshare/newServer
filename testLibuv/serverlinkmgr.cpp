...
const AssociatedSvr_t imsvr::assocSvrArray[] =
{
    ...
    {OFFLINEMSG, "offlinemsg", "/usofflinemsg" },	//for offlinemsg service
	{-1,"",""}
};

ConnMap_t_ g_ServerPhysicalLinkMap;	//Used in net framework to manage physical link. 
...
void CServerLinkMgr::ServerLinkTimeout(
		...
		void * pParam)
{
	ConnMap_t_::iterator it;	
	ConnMap_t_::iterator nextIt;	
		
    std::shared_ptr<CServerLink> pLink = NULL;
	...
	for (it = g_ServerPhysicalLinkMap.begin(); it != g_ServerPhysicalLinkMap.end();) 
	{
		...
		pLink = std::dynamic_pointer_cast<CServerLink>(it->second);
//		pLink->AddRef();
...
void CServerLinkMgr::InitAssocSvr(void)
{
    ...
    nId = m_nAssocSvrId & OFFLINEMSG;
    if (OFFLINEMSG == nId)
    {
        sPath = GetServicePath(OFFLINEMSG);
        m_lsAssocatedPath.push_back(sPath);
    }
